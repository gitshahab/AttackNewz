...
export default function GlobeView() {
    ...
    const load = async () => {
        ...
        // Enrich countries to reflect incident overlays
        const enrichedCountries = countries.map((feature: any) => {
        const countryName = feature.properties.name;
        const incident = mockIncidents.find(i => i.country === countryName);
        return {
            ...feature,
            properties: {
            ...feature.properties,
        ...
        .polygonCapColor((feat: any) =>
            ...
            ? incidentColors[feat.properties.incidentType] || 'rgba(253, 93, 0, 0.7)' // gold fallback
        ...
        .ringColor(d => incidentColors[d.type] || 'rgba(255,255,255,0.6)')
        .ringAltitude(0.01)
        .ringMaxRadius(5)
        .ringPropagationSpeed(2)
        .ringRepeatPeriod(1000)
        // Incident point overlays
        .pointsData(mockIncidents)
        .pointLat(d => d.lat)
        .pointLng(d => d.lng)
        .pointAltitude(() => 0.01)
        .pointColor(d => incidentColors[d.type] || 'rgba(255,255,255,0.8)')
        .pointRadius(() => 1.5)
        .onPointHover((incident: any | null) => {
          setHoveredIncident(incident);
        })
        .onPointClick((incident: any) => {
    ...
    return (
        ...
        {hoveredIncident && (
            ...
            <p className="text-[10px] italic text-gray-300">{hoveredIncident.type}</p>
            </div>
        )}
        </>
    );
